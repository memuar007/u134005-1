...
/*Создание класса с описанием и поведением модуля в системе *Имя класса должно совпадать с ID модуля, вместо точки между именем парнера и именем модуля знак подчеркивания *Класс модуля должен быть наследником системного класса модулей Битрикс CModule*/
...
'OnBasketUpdate'); */
    ...
    {
        /* Пример $em = \Bitrix\Main\EventManager::getInstance(); $em->unRegisterEventHandler('sale', 'OnBasketAdd', self::MODULE_ID, '\CompanyNamespace\Promotions\Connector', 'OnBasketAdd'); $em->unRegisterEventHandler('sale', 'OnBasketUpdate', self::MODULE_ID, '\CompanyNamespace\Promotions\Connector', 'OnBasketUpdate'); */
        return true;
    }
...
if (is_dir($p = $_SERVER['DOCUMENT_ROOT'] . '/local/modules/' . self::MODULE_ID . '/install/components'))
{
    if ($dir = opendir($p))
    {
        while (false !== $item = readdir($dir))
        {
            ...
            $dir0 = opendir($p0); while (false !== $item0 = readdir($dir0))
        {
            if ($item0 == '..' || $item0 == '.')
            {
                continue;
        ...
        closedir($dir0); } closedir($dir); } } return true;
    }
    /*Запускается при нажатии кнопки Удалить на странице Модули административного раздела, осуществляет деинсталляцию модуля*/ /*ОБЯЗАТЕЛЬНЫЙ ДЛЯ РЕАЛИЗАЦИИ МЕТОД*/
    function DoInstall()
    {
        global $APPLICATION;
        $this->InstallFiles();
    ...
    /*Метод запускается при нажатии кнопки Установить на странице Модули административного раздела, осуществляет инсталляцию модуля*/ /*ОБЯЗАТЕЛЬНЫЙ ДЛЯ РЕАЛИЗАЦИИ МЕТОД*/
